...
@router.get("/{user_id}")
def get_user(user_id: Union[str, int], db: Session = Depends(get_db)):
    return crud.get_user_by_id(db, user_id)

@router.post("/{user_id}/update_password")
def update_password(user_id: int, password: schemas.UserUpdatePassword, db: Session = Depends(get_db)):
